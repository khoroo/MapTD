...
namespace lanms {
  ...
  struct Polygon {
    cl::Path poly;
    float score;
    int num_merged; // Score normalizer
  };
  ...
  // Called once after locality-aware merging before final standard NMS is applied
  Polygon normalize_score ( Polygon p ) {
    assert (p.num_merged > 0);
    Polygon q = p;
    q.score = p.score / p.num_merged;
    q.num_merged = 1;
    return q;
  }
  ...
  class PolyMerger {
    public:
      ...
      void add(const Polygon &p_given) {
        Polygon p;
        if (nr_polys > 0) {
          // vertices of two polygons to merge might not be in the same order;
      ...
      Polygon get() const {
        ...
        p.score = score; // total score across all merged polygons
        p.num_merged = nr_polys;
  ...
  // Main function for LANMS
  std::vector<Polygon>
    merge_quadrangle_n9(const float *data, size_t n, float iou_threshold) {
    // data is the array representing the (9*n)x1 blocks of 8 polygon
    // points plus the score for each
      ...
      for (size_t i = 0; i < n; i ++) {
        ...
        Polygon poly{
          ...
          p[8], // score
          1 // initial number of total (merged) polygons represented (by score)
        };

        if (polys.size()) {
          // merge with the last one
          auto &bpoly = polys.back();
          if (should_merge(poly, bpoly, iou_threshold)) { // iou > thresh?
            PolyMerger merger;
            merger.add(bpoly);
            merger.add(poly);
            bpoly = merger.get();
      ...
      // Normalize score before applying NMS
      std::vector<Polygon> normalized_polys;
      std::transform( polys.begin(), polys.end(),
                      std::back_inserter(normalized_polys),
                      lanms::normalize_score );
      return standard_nms(normalized_polys, iou_threshold);
    }
}
